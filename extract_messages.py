...
for after_date, before_date in zip(dates, dates[1:]):
    output_file = os.path.join(BASE_DIR, f"{datetime.strptime(before_date, DATE_FORMAT).strftime('%B_%Y')}.json")
    media_dir = os.path.join(BASE_DIR, f"{datetime.strptime(before_date, DATE_FORMAT).strftime('%B_%Y')} MEDIA")
    commands.append([
